...
# For example, to scan a single image:
...
class DocScanner(object):
    ...
    def get_contour(self, rescaled_image):
        ...
        # If we did not find any valid contours, try a minimum area bounding rectangle
        if not approx_contours and cnts:
            rect = cv2.minAreaRect(cnts[0])
            box = cv2.boxPoints(rect)
            screenCnt = np.array(box, dtype="int32").reshape(4, 2)
            # Reorder points to ensure consistent top-left, top-right, bottom-right, bottom-left order
            screenCnt = transform.order_points(screenCnt)
            screenCnt = np.array([[p] for p in screenCnt], dtype="int32").reshape(4, 2)
        elif not approx_contours:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", help="Output directory", default="output")
